...
void Capacitor::stamp(std::shared_ptr<Matrix<std::complex<double>>> circuit_matrix, std::shared_ptr<Matrix<std::complex<double>>> output_matrix, 
    int num_vsources, double frequency, double prev_frequency) {
    size_t node_1 = terminals_int[0];
    size_t node_2 = terminals_int[1];

    std::complex<double> conductance = std::complex<double>(0.0, 2.0*M_PI*value*(frequency-prev_frequency));
