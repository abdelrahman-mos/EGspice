...
void VCVS::stamp(std::shared_ptr<Matrix<double>> circuit_matrix, std::shared_ptr<Matrix<double>> output_matrix, int num_vsources, int num_inductors) {
    ...
    if (node_3 != 0) {
        (*circuit_matrix)[stamp_index][node_3-1] -= value;
    }

    if (node_4 != 0) {
        (*circuit_matrix)[stamp_index][node_4-1] += value;
    }
}

void VCVS::stamp(std::shared_ptr<Matrix<std::complex<double>>> circuit_matrix, std::shared_ptr<Matrix<std::complex<double>>> output_matrix, int num_vsources, double frequency) {
    ...
    if (node_3 != 0) {
        (*circuit_matrix)[stamp_index][node_3-1] -= ac_val;
    }
